...
This function is an aggregation function similar to `sum ()`. It links multiple rows of results in the result set into a string. The second parameter, `sep`, is a connection symbol between strings, which can be omitted. This function usually needs to be used with `group by` statements.

It support `order by` for reordering multi-row results. Reordering and aggregation columns can be different.
