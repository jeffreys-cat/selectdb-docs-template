...
It converts an array column to multiple rows.  If `array` is null or empty, the returned `explode_outer` part will be NULL.  will return NULL. Both`explode` and `explode_outer`  keep the nested NULL elements of `array`.
