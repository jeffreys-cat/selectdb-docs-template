...
An Export job will be split into multiple subtasks (execution plans) for execution. The number of query plans that need to be executed depends on the total number of tablets and the number of tablets that can be allocated to a query plan.
...
- Exporting a large amount of data at a time is not recommended. We recommend that you export  dozens of GB of data in one Export job. An overly large export results in more junk files and higher retry costs. If the amount of table data is too large, we suggest that you export by partition.
